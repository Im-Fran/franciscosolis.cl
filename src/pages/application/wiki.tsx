...
import {flattenWiki, useApplicationWiki, useApplicationWikiPage} from "@/lib/pages/content.ts";
...
import {WikiSidebar} from "@/pages/application/components/wiki-sidebar.tsx";
...
export const ApplicationWiki = () => {
  ...
  const document = useApplicationWikiPage(slug, known);

  if (tree.loading) {
    return (
      <div className="flex justify-center py-20">
        <Spinner size={24} label={t("application:loading")}/>
      </div>
    );
  }
  ...
  if (ordered.length === 0) {
    return (
      <p className="flex items-center justify-center gap-2 py-16 text-sm text-neutral-500">
        <BookOpen size={18}/> {t("application:wiki.empty")}
      </p>
    );
  }

  /* No page asked for, or one that is not published: open the first entry instead of a dead end. */
  if (!known) return <Navigate to={applicationRoute.wikiPage(slug, ordered[0].slug)} replace/>;
      ...
      <article className="min-w-0">
        {document.loading ? (
          <div className="flex justify-center py-16">
            <Spinner size={22} label={t("application:loading")}/>
          </div>
        ) : document.error ? (
          ...
          <>
            ...
            {document.data.updated_at && (
              <p className="mt-2 text-[13px] text-neutral-600">
                {t("application:wiki.updated", {date: formatDate(document.data.updated_at, i18n.language)})}
              </p>
            )}
            {document.data.body?.trim() ? (
              <ApplicationProse source={document.data.body} className="mt-8"/>
            ) : (
              <p className="mt-8 text-sm text-neutral-500">{t("application:wiki.page_empty")}</p>
            )}
          </>
        ) : null}
      </article>
