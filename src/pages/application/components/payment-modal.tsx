...
import type {Application, ApplicationAccess} from "@/lib/pages/types.ts";
...
type PaymentModalProps = {
  ...
  /**
   * Continue without paying. Only ever passed for an application that allows it — the button is
...
/**
 * The offer to pay for an application.
 *
 ...
 *   application quietly becoming free for whoever cleared it once.
 ...
 */
export const PaymentModal = ({open, onClose, application, access, onSkip}: PaymentModalProps) => {
  ...
  const pay = async () => {
    ...
    setBusy(true);
    try {
      const checkout = await pagesContent.checkout(application.slug, {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={donation ? t("application:payment.support_title", {name: application.name}) : t("application:payment.buy_title", {name: application.name})}
    >
      <div className="flex flex-col gap-5">
        <p className="text-sm text-neutral-400">
          {donation ? t("application:payment.support_body") : t("application:payment.buy_body")}
        </p>

        {donation ? (
          <label className="flex flex-col gap-2">
            <span className="text-[13px] text-neutral-400">
              {t("application:payment.amount_label", {currency: pricing.currency})}
            </span>
            <Input
              type="number"
              inputMode="numeric"
            ...
            <span id="payment-minimum" className="text-[12px] text-neutral-500">
              {t("application:payment.minimum", {amount: formatAmount(minimum, pricing.currency, locale)})}
            </span>
          </label>
        ) : (
        ...
        {/*
          ...
          * application") and is worth more than a generic line would be.
          */}
        {error && (
          <Alert tone="error">
            {error === "network"
        ...
        <div className="flex flex-col gap-3">
          <Button onClick={() => void pay()} disabled={busy || !amountValid}>
            ...
            {status === "authenticated"
              ? t("application:payment.pay")
              : t("application:payment.sign_in_and_pay")}
          </Button>

          {/*
            * The opt-out, stated in full. It only exists when the service says the application
          ...
          {onSkip && (
            <>
              <Button variant="ghost" onClick={onSkip}>
                <Gift size={16}/> {t("application:payment.skip")}
              </Button>
              <p className="text-center text-[12px] text-neutral-500">{t("application:payment.skip_hint")}</p>
            </>
          )}

          {!onSkip && (
            <p className="flex items-start gap-2 text-[12px] text-neutral-500">
              <LockKey size={14} className="mt-0.5 shrink-0"/> {t("application:payment.paid_hint")}
            </p>
          )}
        </div>

        <p className="text-center text-[12px] text-neutral-600">{t("application:payment.provider")}</p>
      </div>
    </Modal>
  );
};
