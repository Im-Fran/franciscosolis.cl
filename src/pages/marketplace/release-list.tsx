...
export const ReleaseList = () => {
  ...
  const columns = useMemo<Column<ProductRelease>[]>(
    () => [
      {
        key: "version",
        header: t("marketplace_admin:releases.columns.version"),
        className: "w-28",
        cell: (row) => <span className="font-mono text-[13px] text-accent-300">{row.version}</span>,
      },
      {
        key: "title",
        header: t("marketplace_admin:releases.columns.title"),
        cell: (row) => <span className="text-sm text-text">{row.title}</span>,
      },
      {
        key: "status",
        header: t("marketplace_admin:releases.columns.status"),
        className: "w-32",
        cell: (row) => <StatusBadge status={row.status}/>,
      },
      {
        key: "released_at",
        header: t("marketplace_admin:releases.columns.released"),
        className: "w-36",
        cell: (row) =>
      ...
      {
        key: "links",
        header: t("marketplace_admin:releases.columns.links"),
        className: "w-20",
        hideBelowLg: true,
      ...
      {
        key: "actions",
        header: <span className="sr-only">{t("marketplace_admin:releases.columns.actions")}</span>,
        ...
        cell: (row) => (
          <div className="flex items-center justify-end gap-1">
            <Button
              ...
              size="icon"
              asChild
              aria-label={t("marketplace_admin:releases.edit_aria", {version: row.version})}
              className="size-9"
            >
            ...
            <Button
              ...
              aria-label={t("marketplace_admin:releases.delete_aria", {version: row.version})}
  ...
  return (
    <>
      <PageHeader
        title={t("marketplace_admin:releases.title")}
        description={t("marketplace_admin:releases.description", {name: product.data?.name ?? ""})}
        back={{to: marketplaceRoute.list, label: t("marketplace_admin:editor.back")}}
        actions={
          <Button asChild>
            <Link to={marketplaceRoute.releaseNew(id)}>
              <Plus size={16}/> {t("marketplace_admin:releases.new")}
            </Link>
          </Button>
        }
      />
      ...
      <Panel
        title={t("marketplace_admin:releases.panel_title")}
        description={total > 0 ? t("marketplace_admin:releases.count", {count: total}) : undefined}
        action={
      ...
      >
        ...
        >
          {total === 0 ? (
            <EmptyState
              icon={<Note size={28}/>}
              title={t("marketplace_admin:releases.empty_title")}
              description={t("marketplace_admin:releases.empty_description")}
              action={
                <Button asChild>
                  <Link to={marketplaceRoute.releaseNew(id)}>
                    <Plus size={16}/> {t("marketplace_admin:releases.empty_action")}
                  </Link>
                </Button>
              }
            />
            ...
            <DataTable
              ...
              caption={t("marketplace_admin:releases.caption")}
            />
          )}
        </PanelState>
      </Panel>
      ...
      <ConfirmDialog
        ...
        body={t("marketplace_admin:releases.delete_body", {version: pendingDelete?.version ?? ""})}
