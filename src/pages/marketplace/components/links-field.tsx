...
import {LINK_KINDS, MAX_LINKS} from "@/lib/marketplace/types.ts";
import type {ProductLink} from "@/lib/marketplace/types.ts";

/**
 * The link editor behind a product's header buttons and a release note's.
 *
 ...
 * button on every product, without an editor choosing an icon. `other` is the escape hatch, and
          ...
          >
            {LINK_KINDS.map((kind) => (
              <option key={kind} value={kind}>
                {t(`product:links.${kind}`, {defaultValue: kind})}
              </option>
            ))}
          </Select>
