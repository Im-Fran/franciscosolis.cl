...
import {ArrowSquareOut, GlobeSimple, List, SignOut, UserCircle, X} from "@phosphor-icons/react";
...
import {buildNav} from "@/pages/auth/admin/components/admin-nav.ts";

const Nav = ({onNavigate}: {onNavigate?: () => void}) => {
  const {t} = useTranslation(["auth_admin", "admin"]);
  const {can} = useAdmin();
  const sections = buildNav(can);

  return (
    <nav className="flex flex-col gap-6" aria-label={t("auth_admin:nav.label")}>
...
/**
 * The frame every console screen sits in: the header, the section navigation and the one place
 * transient confirmations are announced from.
 *
      ...
      <header className="sticky top-0 z-30 border-b border-neutral-800 bg-bg/90 backdrop-blur-sm">
        <div className="mx-auto flex w-full max-w-[1600px] flex-wrap items-center gap-3 px-4 py-3">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {user && (
              <span className="hidden items-center gap-2 rounded-[var(--radius-md)] border border-neutral-800 py-1.5 pr-3 pl-1.5 xl:inline-flex">
