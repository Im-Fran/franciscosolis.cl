...
import {PURCHASE_STATUSES, SALE_SOURCES} from "@/lib/pages/types.ts";
import type {Sale} from "@/lib/pages/types.ts";
...
import {RecordSaleDialog} from "@/pages/cms/pages/components/record-sale-dialog.tsx";
import type {RecordSalePayload} from "@/pages/cms/pages/components/record-sale-dialog.tsx";

/**
 * The takings of one application: what came in, how, and what went back out.
 *
...
export const SalesList = () => {
  ...
  const sales = useResource(
    useCallback((signal: AbortSignal) => pagesApi.sales.list(id, filters, signal), [id, filters]),
  );
  const summary = useResource(
    useCallback((signal: AbortSignal) => pagesApi.sales.summary(id, filters, signal), [id, filters]),
  );

  const record = useMutation(
    useCallback((payload: RecordSalePayload) => pagesApi.sales.create(id, payload), [id]),
  );
  ...
  const columns = useMemo<Column<Sale>[]>(
    () => [
      ...
      {
        key: "amount",
        header: t("cms_pages:sales.columns.amount"),
        className: "w-32",
        cell: (row) => (
          <span className="flex flex-col">
      ...
      {
        key: "source",
        header: t("cms_pages:sales.columns.source"),
        className: "w-40",
        cell: (row) => (
      ...
      {
        key: "status",
        header: t("cms_pages:sales.columns.status"),
        className: "w-32",
        cell: (row) => <StatusBadge status={row.status}/>,
      },
      {
        key: "withdrawal",
        header: t("cms_pages:sales.columns.withdrawal"),
        className: "w-36",
        hideBelowLg: true,
        /*
        ...
        cell: (row) =>
          row.status !== "approved" || !row.withdrawal?.deadline ? (
            ...
            <span className="text-[13px] text-amber-300">
              {t("cms_pages:sales.withdrawal_left", {count: row.withdrawal.days_left ?? 0})}
            </span>
          ) : (
            <span className="text-[13px] text-neutral-500">{t("cms_pages:sales.withdrawal_over")}</span>
          ),
      },
    ],
    [locale, t],
  ...
  const submitSale = async (payload: RecordSalePayload) => {
    ...
    notify(t("cms_pages:sales.record.done"));
    setRecording(false);
    sales.reload();
    summary.reload();
  };
  ...
  return (
    <>
      <PageHeader
        title={t("cms_pages:sales.title")}
        description={t("cms_pages:sales.description", {name: application.data?.name ?? ""})}
        back={{to: pagesRoute.list, label: t("cms_pages:editor.back")}}
        actions={
          <Button onClick={() => setRecording(true)}>
            <Plus size={16}/> {t("cms_pages:sales.record.open")}
          </Button>
        }
      />
      ...
      <Panel
        title={t("cms_pages:sales.totals.title")}
        description={t("cms_pages:sales.totals.description")}
        className="mb-6"
        action={
            ...
            {[
              ...
              <div key={key} className="rounded-[var(--radius-md)] bg-neutral-900/60 px-4 py-3">
                <dt className="text-[12px] tracking-wide text-neutral-500 uppercase">
                  {t(`cms_pages:sales.totals.${key}`)}
                </dt>
                <dd className={accent ? "mt-1 font-display text-2xl text-accent-200" : "mt-1 font-display text-2xl text-text"}>
                  {value}
                </dd>
                <p className="mt-1 text-[12px] text-neutral-500">{t(`cms_pages:sales.totals.${key}_hint`)}</p>
              </div>
            ))}
          </dl>
          <p className="mt-4 text-[13px] text-neutral-400">
            {t("cms_pages:sales.totals.buyers", {count: totals?.buyers ?? 0})}
          </p>
        </PanelState>
      </Panel>

      <Panel
        title={t("cms_pages:sales.panel_title")}
        description={sales.data ? t("cms_pages:sales.count", {count: sales.data.length}) : undefined}
        action={
      ...
      >
        <div className="mb-5 grid gap-3 sm:grid-cols-3">
          <Field label={t("cms_pages:sales.filters.status")} htmlFor="sales-status">
            <Select id="sales-status" value={status} onChange={(event) => setStatus(event.target.value)}>
              ...
              {PURCHASE_STATUSES.map((entry) => (
                <option key={entry} value={entry}>
                  {t(`cms_pages:sales.statuses.${entry}`, {defaultValue: entry})}
                </option>
              ))}
            </Select>
          </Field>
          <Field label={t("cms_pages:sales.filters.source")} htmlFor="sales-source">
            <Select id="sales-source" value={source} onChange={(event) => setSource(event.target.value)}>
              <option value="">{t("cms_pages:sales.filters.any")}</option>
              {SALE_SOURCES.map((entry) => (
                <option key={entry} value={entry}>
                  {t(`cms_pages:sales.sources.${entry}`)}
                </option>
              ))}
            </Select>
          </Field>
          <Field label={t("cms_pages:sales.filters.email")} htmlFor="sales-email">
            <Input
              id="sales-email"
              type="search"
              value={email}
        ...
        >
          {(sales.data?.length ?? 0) === 0 ? (
            <EmptyState
              ...
              title={t("cms_pages:sales.empty_title")}
              description={t("cms_pages:sales.empty_description")}
              action={
                <Button onClick={() => setRecording(true)}>
                  <Plus size={16}/> {t("cms_pages:sales.record.open")}
                </Button>
            ...
            <DataTable
              ...
              rows={sales.data ?? []}
              rowKey={(row) => row.id}
              onRowClick={(row) => navigate(pagesRoute.saleItem(id, row.id))}
              caption={t("cms_pages:sales.caption")}
            />
          )}
        </PanelState>
      </Panel>
