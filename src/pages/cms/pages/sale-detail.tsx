...
import {REFUND_REASONS} from "@/lib/pages/types.ts";
import type {RefundReason, Voucher} from "@/lib/pages/types.ts";
...
import {EnvironmentBadge, SourceBadge} from "@/pages/cms/pages/components/sale-badges.tsx";
...
export const SaleDetail = () => {
  ...
  const detail = useResource(
    useCallback((signal: AbortSignal) => pagesApi.sales.get(id, saleId, signal), [id, saleId]),
  );

  const refund = useMutation(
    useCallback(
      (payload: {reason: RefundReason; amount?: number; notify?: boolean}) =>
        pagesApi.sales.refund(id, saleId, payload),
      [id, saleId],
    ),
  );
  const save = useMutation(
    useCallback(
      (payload: {email?: string; note?: string | null}) => pagesApi.sales.update(id, saleId, payload),
      [id, saleId],
    ),
  );
  const issue = useMutation(useCallback(() => pagesApi.vouchers.issue(id, saleId), [id, saleId]));
  const send = useMutation(
    useCallback(
      (voucherId: string, address?: string) => pagesApi.vouchers.send(id, voucherId, address ? {email: address} : {}),
      [id],
    ),
  );
  const voidVoucher = useMutation(
    useCallback((voucherId: string) => pagesApi.vouchers.void(id, voucherId), [id]),
  );
  ...
  const facts = sale
    ? [
        ...
        {key: "kind", value: t(`cms_pages:sales.kinds.${sale.kind}`, {defaultValue: sale.kind})},
          ...
          value:
            sale.withdrawal?.deadline == null
              ...
              : sale.withdrawal.within_period
                ? t("cms_pages:sales.withdrawal_left", {count: sale.withdrawal.days_left ?? 0})
                : t("cms_pages:sales.withdrawal_over"),
        },
        {
          key: "refund",
          value:
            sale.refunded_at == null
              ...
              : `${formatAmount(sale.refunded_amount ?? sale.amount, sale.currency, locale)} · ${t(
                  `cms_pages:sales.reasons.${sale.refund_reason ?? "other"}`,
                  {defaultValue: sale.refund_reason ?? ""},
                )}`,
        },
        {key: "account", value: sale.linked_to_account ? sale.user_id : t("cms_pages:sales.unlinked")},
        {key: "recorded_by", value: sale.created_by ?? t("cms_pages:sales.recorded_by_provider")},
      ]
    : [];
  ...
  const runVoid = async (voucher: Voucher) => {
    ...
    notify(t("cms_pages:vouchers.voided"));
    detail.reload();
  };
  ...
  const voucherColumns: Column<Voucher>[] = [
    ...
    {
      key: "sent",
      header: t("cms_pages:vouchers.columns.sent"),
      className: "w-40",
      hideBelowLg: true,
      cell: (row) =>
        row.sent_count ? (
          <span className="text-[13px] text-neutral-400">
            {t("cms_pages:vouchers.sent_times", {count: row.sent_count})} ·{" "}
            {formatDateTime(row.last_sent_at, locale)}
          </span>
        ) : (
          <span className="text-[13px] text-amber-300">{t("cms_pages:vouchers.never_sent")}</span>
        ),
    },
    {
      key: "actions",
      ...
      cell: (row) => (
        <div className="flex items-center justify-end gap-1">
          ...
          <Button
            ...
            aria-label={t("cms_pages:vouchers.void_aria", {number: row.number})}
  ...
  const runIssue = async () => {
    ...
    if (!outcome.ok) return;
    notify(t("cms_pages:vouchers.issued"));
    detail.reload();
  };
  ...
  const runRefund = async () => {
    ...
    if (!outcome.ok) return;
    notify(t("cms_pages:sales.refund.done"));
    setRefunding(false);
    setPartial("");
    detail.reload();
  };
  ...
  return (
    <>
      <PageHeader
        title={sale ? sale.email : t("cms_pages:sales.detail.title")}
        description={t("cms_pages:sales.detail.description")}
        back={{to: pagesRoute.sales(id), label: t("cms_pages:sales.detail.back")}}
        actions={
          <>
            ...
            >
              <ArrowUUpLeft size={16}/> {t("cms_pages:sales.refund.open")}
            </Button>
          </>
        }
      />
      ...
      <Panel title={t("cms_pages:sales.detail.panel_title")} className="mb-6">
        <PanelState
        ...
        >
            ...
            {facts.map(({key, value}) => (
              <div key={key} className="flex flex-col gap-0.5 border-b border-neutral-800/60 pb-2">
                <dt className="text-[12px] tracking-wide text-neutral-500 uppercase">
                  {t(`cms_pages:sales.detail.facts.${key}`)}
                </dt>
          ...
          {!refundable && detail.data?.refund.reason && (
            <p className="mt-4 text-[13px] text-neutral-400">
              {t(`cms_pages:sales.refund.blocked_${detail.data.refund.reason}`)}
            </p>
          )}
        </PanelState>
      </Panel>
      ...
      <Panel
        title={t("cms_pages:sales.detail.edit_title")}
        description={t("cms_pages:sales.detail.edit_description")}
        className="mb-6"
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <Field
            label={t("cms_pages:sales.detail.facts.email")}
            htmlFor="sale-edit-email"
            hint={t("cms_pages:sales.detail.email_hint")}
          >
            <Input
              id="sale-edit-email"
              type="email"
          ...
          <Field
            label={t("cms_pages:sales.detail.facts.note")}
            htmlFor="sale-edit-note"
            hint={t("cms_pages:sales.detail.note_hint")}
          >
            <Textarea
              id="sale-edit-note"
              rows={2}
      ...
      <Panel
        title={t("cms_pages:vouchers.panel_title")}
        description={t("cms_pages:vouchers.panel_description")}
        action={
          <Button variant="secondary" size="sm" onClick={() => void runIssue()} disabled={issue.pending}>
            <Receipt size={14}/> {t("cms_pages:vouchers.issue")}
          </Button>
        }
      >
        {issue.error && (
          <p className="mb-3 text-[13px] text-red-400">
            {issue.status === 502 ? t("cms_pages:vouchers.issue_send_failed") : issue.error}
          </p>
        )}
        ...
        {vouchers.length === 0 ? (
          <EmptyState
            ...
            description={t("cms_pages:vouchers.empty_description")}
          />
        ) : (
          <DataTable
      ...
      <Modal open={refunding} onClose={() => setRefunding(false)} title={t("cms_pages:sales.refund.title")}>
        <p className="mb-5 text-[13px] leading-relaxed text-neutral-400">{t("cms_pages:sales.refund.body")}</p>
        <div className="grid gap-4">
          <Field
            label={t("cms_pages:sales.refund.reason")}
            htmlFor="refund-reason"
            hint={t("cms_pages:sales.refund.reason_hint")}
          >
            <Select
              id="refund-reason"
              value={reason}
            ...
            >
              {REFUND_REASONS.map((entry) => (
                <option key={entry} value={entry}>
                  {t(`cms_pages:sales.reasons.${entry}`)}
                </option>
              ))}
            </Select>
          </Field>
          <Field
            label={t("cms_pages:sales.refund.amount")}
            htmlFor="refund-amount"
            hint={t("cms_pages:sales.refund.amount_hint", {
          ...
          <label className="flex items-center gap-2 text-[13px] text-neutral-300">
            ...
            />
            {t("cms_pages:sales.refund.notify")}
          </label>
        </div>
        ...
        <div className="mt-6 flex flex-wrap justify-end gap-2">
          ...
          <Button onClick={() => void runRefund()} disabled={refund.pending}>
            {refund.pending ? t("cms_pages:sales.refund.pending") : t("cms_pages:sales.refund.submit")}
          </Button>
        </div>
      </Modal>

      <Modal open={sendTo !== null} onClose={() => setSendTo(null)} title={t("cms_pages:vouchers.send_title")}>
        <p className="mb-5 text-[13px] leading-relaxed text-neutral-400">{t("cms_pages:vouchers.send_body")}</p>
        <Field
          label={t("cms_pages:vouchers.send_to")}
          htmlFor="voucher-send-to"
          hint={t("cms_pages:vouchers.send_to_hint")}
        >
          <Input
            id="voucher-send-to"
            type="email"
        ...
        <div className="mt-6 flex flex-wrap justify-end gap-2">
          ...
          <Button onClick={() => void runSend()} disabled={send.pending}>
            {send.pending ? t("cms_pages:vouchers.sending") : t("cms_pages:vouchers.send")}
          </Button>
        </div>
      </Modal>
    </>
