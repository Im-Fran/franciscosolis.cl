...
import type {WikiPage} from "@/lib/pages/types.ts";
...
/**
 * One application's wiki, as a flat list in sidebar order.
 *
...
export const WikiList = () => {
  ...
  const pages = useResource(useCallback((signal: AbortSignal) => pagesApi.wiki.list(id, {}, signal), [id]));
  const remove = useMutation(useCallback((pageId: string) => pagesApi.wiki.remove(id, pageId), [id]));
  const reorder = useMutation(
    useCallback((items: {id: string; position: number}[]) => pagesApi.wiki.reorder(id, items), [id]),
  );
  ...
  const columns = useMemo<Column<WikiPage>[]>(
    () => [
      {
        key: "title",
        header: t("cms_pages:wiki.columns.page"),
        cell: (row) => (
      ...
      {
        key: "parent",
        header: t("cms_pages:wiki.columns.section"),
        className: "w-44",
        hideBelowLg: true,
        cell: (row) =>
          row.parent_id ? (
            <span className="text-[13px] text-neutral-400">
              {titleById.get(row.parent_id) ?? t("cms_pages:wiki.unknown_section")}
            </span>
          ) : (
            <span className="text-[13px] text-neutral-600">{t("cms_pages:wiki.top_level")}</span>
          ),
      },
      {
        key: "status",
        header: t("cms_pages:wiki.columns.status"),
        className: "w-32",
        cell: (row) => <StatusBadge status={row.status}/>,
      },
      {
        key: "actions",
        header: <span className="sr-only">{t("cms_pages:wiki.columns.actions")}</span>,
  ...
  return (
    <>
      <PageHeader
        title={t("cms_pages:wiki.title")}
        description={t("cms_pages:wiki.description", {name: application.data?.name ?? ""})}
        ...
        >
          {total === 0 ? (
            ...
            <SortableList
              ...
              label={t("cms_pages:wiki.reorder_label")}
              renderItem={(row) => (
            ...
            <DataTable
              ...
              rows={rows}
              rowKey={(row) => row.id}
              onRowClick={(row) => navigate(pagesRoute.wikiItem(id, row.id))}
              caption={t("cms_pages:wiki.caption")}
            />
          )}
        </PanelState>
      </Panel>
      ...
      <ConfirmDialog
        ...
        body={t("cms_pages:wiki.delete_body", {title: pendingDelete?.title ?? ""})}
