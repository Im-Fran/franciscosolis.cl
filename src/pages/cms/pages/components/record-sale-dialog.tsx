...
import {MANUAL_SALE_SOURCES} from "@/lib/pages/types.ts";
import type {Sale, Voucher} from "@/lib/pages/types.ts";
...
export const RecordSaleDialog = ({open, onClose, onSubmit, mutation}: RecordSaleDialogProps) => {
  const {t} = useTranslation(["cms_pages", "admin"]);
  ...
  return (
    <Modal open={open} onClose={onClose} title={t("cms_pages:sales.record.title")}>
      <p className="mb-5 text-[13px] leading-relaxed text-neutral-400">{t("cms_pages:sales.record.body")}</p>

      <div className="grid gap-4 sm:grid-cols-2">
        <Field label={t("cms_pages:sales.record.email")} htmlFor="sale-email" hint={t("cms_pages:sales.record.email_hint")}>
          <Input
            id="sale-email"
            type="email"
            value={email}
        ...
        <Field label={t("cms_pages:sales.record.source")} htmlFor="sale-source" hint={t("cms_pages:sales.record.source_hint")}>
        ...
        <Field label={t("cms_pages:sales.record.amount")} htmlFor="sale-amount" hint={t("cms_pages:sales.record.amount_hint")}>
          <Input
            id="sale-amount"
            type="number"
            min={0}
        ...
        <Field label={t("cms_pages:sales.record.kind")} htmlFor="sale-kind" hint={t("cms_pages:sales.record.kind_hint")}>
          <Select
            id="sale-kind"
            value={kind}
            onChange={(event) => setKind(event.target.value === "donation" ? "donation" : "purchase")}
          >
            <option value="purchase">{t("cms_pages:sales.kinds.purchase")}</option>
            <option value="donation">{t("cms_pages:sales.kinds.donation")}</option>
          </Select>
        </Field>

        <Field label={t("cms_pages:sales.record.occurred_at")} htmlFor="sale-date" hint={t("cms_pages:sales.record.occurred_at_hint")}>
          <Input
            id="sale-date"
            type="datetime-local"
            value={occurredAt}
        ...
        <Field label={t("cms_pages:sales.record.reference")} htmlFor="sale-reference" hint={t("cms_pages:sales.record.reference_hint")}>
          <Input id="sale-reference" value={reference} onChange={(event) => setReference(event.target.value)}/>
        </Field>

        <Field
          label={t("cms_pages:sales.record.note")}
          htmlFor="sale-note"
          hint={t("cms_pages:sales.record.note_hint")}
          className="sm:col-span-2"
        >
          <Textarea id="sale-note" rows={2} value={note} onChange={(event) => setNote(event.target.value)}/>
        </Field>

        <Field label={t("cms_pages:sales.record.locale")} htmlFor="sale-locale" hint={t("cms_pages:sales.record.locale_hint")}>
          <Select id="sale-locale" value={locale} onChange={(event) => setLocale(event.target.value)}>
            <option value="es">{t("cms_pages:sales.locales.es")}</option>
            <option value="en">{t("cms_pages:sales.locales.en")}</option>
          </Select>
        </Field>
        ...
        <div className="flex flex-col justify-end gap-2 text-[13px] text-neutral-300">
          <label className="flex items-center gap-2">
            ...
            />
            {t("cms_pages:sales.record.issue_voucher")}
          </label>
          <label className="flex items-center gap-2 text-neutral-400">
            <input
              ...
              className="size-4 accent-accent-500"
            />
            {t("cms_pages:sales.record.notify")}
          </label>
        </div>
      </div>
      ...
      {mutation.error && (
        <p className="mt-4 text-[13px] text-red-400">
          {/*
            ...
            */}
          {mutation.status === 502 ? t("cms_pages:sales.record.sent_failed") : mutation.error}
        </p>
      )}
      ...
      <div className="mt-6 flex flex-wrap justify-end gap-2">
        ...
        <Button onClick={submit} disabled={invalid || mutation.pending}>
          {mutation.pending ? t("admin:common.creating") : t("cms_pages:sales.record.submit")}
        </Button>
      </div>
    </Modal>
  );
