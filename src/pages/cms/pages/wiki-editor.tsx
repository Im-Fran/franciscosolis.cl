...
export const WikiEditor = () => {
  ...
  const record = useResource(
    useCallback(
      (signal: AbortSignal) => (pageId ? pagesApi.wiki.get(id, pageId, signal) : Promise.resolve(null)),
      [id, pageId],
    ),
  );
  ...
  const siblings = useResource(useCallback((signal: AbortSignal) => pagesApi.wiki.list(id, {}, signal), [id]));

  const save = useMutation(
    useCallback(
      (payload: WikiPayload) =>
        pageId
          ? pagesApi.wiki.update(id, pageId, payload)
          : pagesApi.wiki.create(id, {...payload, title: payload.title ?? ""}),
      [id, pageId],
    ),
  );

  const remove = useMutation(useCallback(() => pagesApi.wiki.remove(id, pageId ?? ""), [id, pageId]));
  ...
  const persist = async () => {
    ...
    if (!pageId) {
      ...
      setBaseline(form);
      navigate(pagesRoute.wikiItem(id, outcome.data.id), {replace: true});
      return;
    }
  ...
  const confirmDelete = async () => {
    ...
    navigate(pagesRoute.wiki(id), {replace: true});
  };

  const heading = pageId ? loaded?.title || t("cms_pages:wiki.title_edit") : t("cms_pages:wiki.title_new");
  const formId = "wiki-editor-form";

  const header = (
    <PageHeader
      title={heading}
      description={t("cms_pages:wiki.editor_description")}
      back={{to: pagesRoute.wiki(id), label: t("cms_pages:wiki.back")}}
      actions={
        <>
  ...
  if (pageId && (record.loading || record.error)) {
    return (
      <>
        {header}
        <Panel title={t("cms_pages:wiki.page")}>
          <PanelState
  ...
  return (
    <>
        ...
        <Panel title={t("cms_pages:wiki.page")} description={t("cms_pages:wiki.page_hint")}>
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <Field
              ...
              htmlFor="wiki-slug"
              error={errors.slug}
              hint={t("cms_pages:hints.wiki_slug")}
            >
              <Input
                id="wiki-slug"
                value={form.slug}
            ...
            <Field
              label={t("cms_pages:fields.parent")}
              htmlFor="wiki-parent"
              hint={hasChildren ? t("cms_pages:hints.parent_locked") : t("cms_pages:hints.parent")}
            >
              <Select
                id="wiki-parent"
                value={form.parentId}
              ...
              >
                <option value="">{t("cms_pages:wiki.top_level")}</option>
            ...
            <Field
              ...
              htmlFor="wiki-status"
              hint={t(`cms_pages:wiki.status_hint.${form.status}`)}
            >
              <Select
                id="wiki-status"
                value={form.status}
            ...
            <Field
              label={t("cms_pages:fields.icon")}
              htmlFor="wiki-icon"
              error={errors.icon}
              hint={t("cms_pages:hints.wiki_icon")}
      ...
      <ConfirmDialog
        open={askDelete}
        title={t("cms_pages:wiki.delete_title")}
        body={t("cms_pages:wiki.delete_body", {title: form.title.trim()})}
        confirmLabel={remove.pending ? t("admin:common.deleting") : t("cms_pages:wiki.delete_confirm")}
