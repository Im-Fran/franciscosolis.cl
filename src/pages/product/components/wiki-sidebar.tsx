...
import type {WikiNode} from "@/lib/marketplace/types.ts";
...
export const WikiSidebar = ({
  ...
  const link = (node: WikiNode, nested: boolean) => (
    <NavLink
      key={node.id}
      to={productRoute.wikiPage(slug, node.slug)}
      className={({isActive}) =>
        cn(
  ...
  return (
    <nav aria-label={t("product:wiki.nav_label")} className={cn("flex flex-col gap-1", className)}>
      <p className="px-3 pb-2 text-[11px] font-medium tracking-wider text-neutral-600 uppercase">
        {t("product:wiki.nav_title")}
      </p>
      {nodes.map((node) => (
