...
export const ProductLinks = ({links, size = "lg", className}: ApplicationLinksProps) => {
  ...
  const renderLink = (link: ProductLink) => {
    const LinkIcon = ICONS[link.kind] ?? ArrowSquareOut;
    const label = link.label?.trim() || t(`product:links.${link.kind}`, {defaultValue: link.kind});
      ...
      <li key={`${link.kind}:${link.url}`}>
        ...
        {link.kind === "app_store" ? (
          ...
          >
            ...
            <img
              src={appStoreBadge}
              alt={link.label?.trim() || t("product:links.app_store_badge_alt")}
