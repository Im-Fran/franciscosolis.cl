...
import type {ApplicationAccess, DownloadTicket, ReleaseFiles} from "@/lib/pages/types.ts";
 ...
 * payment calls.
 */
export const useApplicationAccess = (slug: string): Resource<ApplicationAccess> => {
  ...
  return useResource(
    useCallback(
      (signal: AbortSignal) => {
         ...
         */
        void status;
        return pagesContent.access(slug, signal);
      },
      [slug, status],
    ),
  );
...
export const useReleaseFiles = (slug: string, version: string): Resource<ReleaseFiles> =>
  useResource(
    useCallback((signal: AbortSignal) => pagesContent.releaseFiles(slug, version, signal), [slug, version]),
  );
 ...
 */
export const usePaymentReturn = (access: Resource<ApplicationAccess>): PaymentReturn => {
